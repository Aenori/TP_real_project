...
<body>
	<div id='main_section'>
		...
		<form id="addFormationForm" action="" method="POST">
			Date: <input type="text" name="date"><br /> Sujet: <input
				type="text" name="sujet" /><br /> Formateur: <input type="text"
				name="formateur" /><br /> PaieFormateur: <input type="integer"
				name="paieFormateur" /><br /> <input type="submit" value="Submit" />
		</form>
	</div>
	<script>
	...
	function updateFormationTable(data){
		...
		document.getElementById("formation_table_body").innerHTML += new_line;
	};
      
	document.getElementById('addFormationForm').submit(function(event) {
  		...
  		$.ajax({
    		...
    		url: 'FormationAjax',
    		data: form.serialize()
  		}).done(function(addFormationResultData) {
  			console.log('Call to ajax worked !')
  			console.log(JSON.stringify(addFormationResultData));
  			
  			updateFormationTable(addFormationResultData);
    		// Optionally alert the user of success here...
  		}).catch(function(e) {
  			console.log('Call to ajax failed !')
  			console.log(e);
  		});
	});
      </script>
